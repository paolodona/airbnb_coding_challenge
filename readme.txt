...
OUTPUT:
Your program will use the input data to perform searches for available properties.  Each search specifies a location and a date range.  Your program should return the 10 cheapest, nearby properties for that date range.  For the geographic filter, use a bounding box that is 2 degrees square in total (ie, +/- 1.0 degrees from each coordinate).  If a property is unavailable for any date during the range, it is not a valid result.  If a property has a variable price specified during the range, that variable price overrides the base nightly price.  The total price is the sum of the nightly prices for the entire stay.  The program should return the cheapest available properties, in order, up to a max of 10.  
